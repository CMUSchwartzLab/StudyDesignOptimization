...
def run_align_sort_index(data_directory, data_name, ref, threads=4, normal=1, tumor_num=0, sample_num=0, single_cell_flag = False, single_cell_num = 0):
    ...
    checkpath(result_dir)
    input_file = [data_dir+'_1.fastq.gz', data_dir + '_2.fastq.gz']
    output_file = result_dir +'/tumor.sam'
    aligner = 'bowtie2'
    if normal == 1:
...
def run_variant(data_directory, data_name, ref, tumor_num, sample_num, \
    ...
    if not single_cell_flag:
        if(tumor_num != -1):
             ...
             tumor_bam = result_dir+'/tumor.sorted.bam'
    else:
        if(tumor_num != -1):
            ...
            print('liquid bio protocol triggered')
            tumor_bam = result_dir + '/tumor.sorted.bam'
    ref = ref
    # 1. call snv
    if snv_caller != 'None':
        if not single_cell_flag:
            if(tumor_num != -1):
                ...
                snv_result_dir = '{}/snv'.format(result_dir)
        else: 
...
def singleLiquidBioalignsortcall(data_directory, data_name, align, threads, snv_caller, cnv_caller, sv_caller, ref_name, align_normal = False): 
    print(data_directory)
    ref = '/projects/schwartzlabscratch/DesignOpt/test_results/ref/{}/{}'.format(ref_name, ref_name)
    run_align_sort_index(data_directory, data_name, ref, normal = 0, tumor_num = -1, sample_num = -1, threads =threads)
    run_variant(data_directory, data_name, ref, tumor_num = -1, sample_num = -1, snv_caller = snv_caller, cnv_caller = cnv_caller, sv_caller = sv_caller, threads = threads, single_cell_flag = False, single_cell_num = 0, align_flag = align_normal)
...
if __name__ == '__main__':
    data_directory_train = '/projects/schwartzlabscratch/LiquidBiopsyData/training_data/'
    data_directory_test = '/projects/schwartzlabscratch/LiquidBiopsyData/test_data/'
    align = 1
    tumor_num = 0
    sample_num = 0
    snv_caller = 'strelka'
    ...
    for i in range(24, 61): 
        data_name = 'SRR282688{}'.format(i)
        singleLiquidBioalignsortcall(data_directory_train, data_name, align, threads, snv_caller, cnv_caller, sv_caller, ref_name)
    for j in range(61, 72):
        data_name = 'SRR282688{}'.format(j)
        singleLiquidBioalignsortcall(data_directory_test, data_name, align, threads, snv_caller, cnv_caller, sv_caller, ref_name)
# if __name__ == '__main__':
